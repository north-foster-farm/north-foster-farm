LEASE: 2026-08-26T22:35Z run-242
...
2026-08-26T21:34Z — No-op on input, run 241. INBOX `sequence:` is still 5
against `inbox-processed: 5` with status `waiting-on-james`, so the
new-input guard fired; I took the lease at 21:34Z, re-verified everything
against the API, and released it. Thirty-fifth consecutive run with
...
thirty-four: the stall has not changed shape since the one I sent, and
