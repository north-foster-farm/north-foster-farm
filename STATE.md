LEASE: 2026-08-21T16:35:21Z run-116
...
2026-08-21T15:35Z — No-op on input, run 115. INBOX `sequence:` is still
5 against `inbox-processed: 5` with status `waiting-on-james`, so the
new-input guard fired for the sixteenth run running. Took the lease at
15:35:07Z, re-verified `main` and all four open PR head SHAs against
the API, released it. Woke 60 minutes after the 14:35Z run — the
cadence is now exactly hourly and has held for seven runs.
...
are too few. Sixteen consecutive hours with nothing to do. The only
