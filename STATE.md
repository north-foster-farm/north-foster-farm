LEASE: 2026-08-20T23:34:34Z run-99
...
2026-08-20T22:34Z — No-op on input, run 98. INBOX `sequence:` is still
5 against `inbox-processed: 5` with status `waiting-on-james`, so the
new-input guard fired. Took the lease at 22:34:47Z, re-verified `main`
and all four open PR head SHAs against the API, released it. Woke ~60
minutes after the 21:34Z run; hourly cadence holds.
