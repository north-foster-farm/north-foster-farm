LEASE: 2026-08-14T22:35:34Z run-22h35
...
2026-08-14T21:35Z — No-op run, same cause as every run since the audit
...
Nothing changed since the 20:35Z run an hour ago. `main` is unchanged at
...
The branch check via `git ls-remote --heads origin` listed the same six
remote heads, `agent/wip-eslint-10` still at 5744535. The container came
up with the full set of remote-tracking refs again, which matches last
run and further supports the read that the `branch -r` gap two runs ago
was a per-container artefact; `ls-remote` stays the answer regardless,
since it does not depend on what the clone happened to fetch.

The stale-clone wrinkle did not reproduce — twenty clean wakes in a row:
`origin/main` came up already at 630fe0e and `git fetch origin main`
moved nothing. Still a quiet stretch rather than evidence of a fix, so
keep fetching before trusting any ref.
